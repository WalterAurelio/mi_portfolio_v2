...
function Proyectos({ showBoxes }: WithBoxes) {
  useGSAP(() => {
    ...
    tl.from('.scrollPanelB', { xPercent: 100 }).from('.scrollPanelC', { xPercent: 100 });
  ...
  return (
    <section id='proyectosSection'>
      <div className='flex -ml-4 lg:-ml-10 w-screen h-lvh lg:h-screen relative proyectosContainer'>
        <div className={cn('pl-4 lg:pl-10 overflow-clip w-full', 'scrollPanelA', { 'bg-red-box': showBoxes })}>
          <ProyectosDescription showBoxes={showBoxes} />
        </div>

        <Proyecto className='min-w-full absolute border-l border-main-black scrollPanelB' />
        <Proyecto className='min-w-full absolute border-l border-main-black scrollPanelC' />
      </div>
    </section>
  );
}
